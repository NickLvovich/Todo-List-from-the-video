class TodoList {
  constructor () {
    ...
    this.removeButton = document.querySelector(".remove-text");
    this.bindEvents();
  }
  ...
  markTodo(id, isCheched) {
    let obj = this.todos.get(id);
    obj.checked = !isCheched;
  ...
  template(item, id) {
    return (`<li class="todo-items ${(item.checked ? "checked" : "")}" 
    data-key="${id}"><input type="checkbox" data-key="${id}" ${(item.checked ? "checked" : "")}/> ${ item.text } </li>`);
  }
  render() {
    let todoElements = [];
